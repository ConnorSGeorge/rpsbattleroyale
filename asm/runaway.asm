...
!main_loop
    ...
    lod rA, [DETECT_PRED]  ; Load detection data for nearest predator
    set rB, rA

    and rB, 1             ; Extract bit 0 to check if predator is nearby
    cmp rB, 1             ; Compare bit 0 of rA to 1 (predator nearby)
    jne !main_loop        ; If no predator, continue looping

    shr rA, 1             ; Extract latitude direction (bit 1)
    set rB, rA
    and rB, 1             ; 1 = North, 0 = South
    cmp rB, 0             ; Compare latitude direction to 0 (South)
    je !move_north        ; Jump to move_north if equal
...
!move_south
    ...
    je !move_east         ; Jump to move_east if equal
    jmp !move_west        ; Otherwise, move West
